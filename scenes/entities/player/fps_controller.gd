...
## Hide all Player visuals from Player camera
func _ready() -> void:
	for child in %View.find_children("*", "VisualInstance3D"):
		child.set_layer_mask_value(1, false)
		child.set_layer_mask_value(2, true)
		

## Handle player movement, crouch, and noclip.
...
## Handles crouch input and adjusts the player collider and camera.
...
## Handles noclip input and movement. Toggles collider and moves freely if enabled.
func _handle_noclip(delta: float) -> bool:
	...
	if not noclip:
		return false
	var speed = _get_move_speed() * noclip_speed_mult
...
## Handles player movement and friction when on the ground.
func _handle_ground_physics(delta: float) -> void:
	var cur_speed_in_wish_dir = self.velocity.dot(wish_dir)
	var add_speed_til_cap = _get_move_speed() - cur_speed_in_wish_dir
	...
	$CameraController.headbob_effect(delta)
	

## Handles player movement and friction when in the air.
...
## Get the Player's intended move speed.
...
## Determine whether a given surface's normal is too steep to move up.
func _is_surface_too_steep(normal: Vector3) -> bool:
	return normal.angle_to(Vector3.UP) > self.floor_max_angle


## Test if the intended movement location is valid.
...
## Handle stair movement going down.
...
## Handle stair movement going up.
